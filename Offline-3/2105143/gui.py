...
class ChainReactionGUI:
    ...
    def draw_panel(self, surface, rect, title=None):
        ...
        title_height = 0
        if title:
            # Title bar - INCREASE HEIGHT HERE
            title_bar_height = 35  # Changed from 25 to 35
            title_rect = (x, y, w, title_bar_height)
            self.draw_rounded_rect(surface, COLORS['accent'], (x, y, w, title_bar_height), 8)
            pygame.draw.rect(surface, COLORS['bg_menu'], (x, y + title_bar_height - 10, w, 10))
            
            title_surf = self.fonts['header'].render(title, True, COLORS['bg_primary'])
            title_x = x + 10
            # CENTER THE TEXT VERTICALLY IN THE NEW HEADER
            title_y = y + (title_bar_height - title_surf.get_height()) // 2
    ...
    def _main_menu(self):
        ...
        options = [
            ("1", "Two Players (P_VS_P)", "Play against a friend"),
            ("2", "Human vs AI (P_VS_AI)", "Challenge the computer"),
            ("3", "AI vs AI Battle (AI_VS_AI)", "Watch AIs compete"),
        ...
        while choosing:
            ...
            panel_height = min(400, HEIGHT - 30)
            panel_x = (WIDTH - panel_width) // 2
            panel_y = (HEIGHT - panel_height) // 2
            panel_rect = (panel_x, panel_y, panel_width, panel_height)
            content_rect = self.draw_panel(self.screen, panel_rect, "Chain Reaction")
    ...
    def _enhanced_test_ai_vs_ai(self):
        ...
        while testing:
            ...
            panel_width = min(450, WIDTH - 20)
            panel_height = min(400, HEIGHT - 20)
            panel_rect = (10, 10, panel_width, panel_height)
            content_rect = self.draw_panel(self.screen, panel_rect, "Enhanced Test Results")
            ...
            p1_results = [
                f"Player 1 (Red): {move1}",
                f"  Time: {time1:.3f}s | Nodes: {stats1['nodes_explored']}",
                f"  Depth: {self.ai_player1_config.depth} | Active Heuristics: {len([h for h, e in self.ai_player1_config.enabled_heuristics.items() if e])}",
            ]
            ...
            p2_results = [
                f"Player 2 (Blue): {move2}",
                f"  Time: {time2:.3f}s | Nodes: {stats2['nodes_explored']}",
                f"  Depth: {self.ai_player2_config.depth} | Active Heuristics: {len([h for h, e in self.ai_player2_config.enabled_heuristics.items() if e])}",
            ]
    ...
    def handle_click(self, mx: int, my: int):
        ...
        if self.mode == MODE_AI_VS_AI or self.animating or self.waiting_for_ai:  # Block clicks during animation or AI thinking
            return
    ...
    def update_ai_vs_ai(self):
        """Update AI vs AI game state"""
         ...
         # Block AI moves during animation
        if not self.ai_vs_ai_running or self.state.game_over or self.animating: 
            return
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    elif event.key == pygame.K_r and self.mode == MODE_AI_VS_AI:

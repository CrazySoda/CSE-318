...
def write_state_with_move(header: str, state: core.GameState, move: tuple[int, int] = None): # type: ignore
    ...
    if move is not None:
        data.append(f"MOVE:{move[0]},{move[1]}")
    
    # Add game state
    data.extend(state.to_lines())
    
    with open(FILE, "w", encoding="utf-8") as f:
        f.write("\n".join(data))

def write_state(header: str, state: core.GameState):
    """Write game state to file (legacy method)."""
    write_state_with_move(header, state)
...
def main():
    ...
    while True:
        try:
            ...
            print(f"  AI chose move: ({r}, {c})")
            
            # Apply move
            explosions = state.apply_move(r, c)
            if explosions:
                print(f"  Explosions at: {explosions}")

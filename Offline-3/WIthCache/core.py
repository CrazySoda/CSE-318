...
class GameState:
    ...
    def from_file(lines: list[str]) -> "GameState":
        ...
        Handles both old format (10 lines) and new format with MOVE: lines.
        """
        # Filter out MOVE: lines and other non-game-state lines
        game_lines = []
        for line in lines:
            line = line.strip()
        ...
        if len(game_lines) != 10:  # metadata + 9 board rows
            raise ValueError(f"Expected 10 game state lines, got {len(game_lines)}. Lines: {game_lines}")
        
        # Parse metadata
        try:
            turns_played, game_over_int, current_player = map(int, game_lines[0].split())
        except ValueError as e:
            raise ValueError(f"Invalid metadata line: '{game_lines[0]}'. Error: {e}")
        ...
        # Parse board (skip metadata line)
        for r, line in enumerate(game_lines[1:]):
            tokens = line.strip().split()
            if len(tokens) != 6:
                raise ValueError(f"Row {r} must have 6 cells, got {len(tokens)}: '{line}'")
            for c, tok in enumerate(tokens):
                ...
                try:
                    ...
                    raise ValueError(f"Invalid token '{tok}' at position ({r}, {c}). Error: {e}")
        return gs
    ...
    def _fast_owner_check(self) -> int:
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                if self.board[r][c].owner:
                    found_owners.add(self.board[r][c].owner)
    ...
    def _place_orb(self, r: int, c: int, player: int,
        ...
        """Queue that performs placement & chain reactions with early win detection."""
        q = deque([(r, c, player)])
        while q:
            cr, cc, p = q.popleft()
            ...
            if will_explode and cell.count >= crit:     
                ...
                if self.turns_played >= 1:  # Only check after at least one full turn
